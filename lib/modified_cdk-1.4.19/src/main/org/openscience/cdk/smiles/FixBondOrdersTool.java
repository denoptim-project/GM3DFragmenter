 ...
 */
	...
	//MFmod: we'll process the rings twice. Thus, the end of the modification is inside the
        // next loop over notSolved rings

        //Does the ring has already double bonds?
        ...
        for (int i = 0; i < ringGroups.size(); i++) 
        {
            ...
            int numAr = 0;
            int numBnds = 0;
            for (int nb=0; nb<bondNos.size(); nb++)
            {
                numBnds++;
                IBond bnd = mNew.getBond(bondNos.get(nb));
                if (bnd.getFlag(CDKConstants.ISAROMATIC))
                    numAr++;
            }

            //Reject if bonds are not reported as aromatic
            if (numAr != numBnds)
            {
                skip.add(true);
            } else {
        ...
        //proper molecule.
        List<Integer> notSolved = new ArrayList<Integer>();
        for (int i = 0; i < ringGroups.size(); i++) 
        {
            //Skip if not defined with only aromatic bonds
            if (skip.get(i))
            {
                continue;
            }
            ...
            for (int x = 0; x < M.getRows(); x++) 
            {
                for (int y = 0; y < M.getCols(); y++) {
                    if (((int) atomNos.get(x)) == ((int) atomNoPairs.get(y)[0])) {
                        M.set(x, y, 1);
                    } else {
                        if ((((int) atomNos.get(x)) == ((int) atomNoPairs.get(y)[1]))) {
                            M.set(x, y, 1);
                        } else {
                            M.set(x, y, 0);
                        }
        ...
        } //end of loop through each group of rings

        //Try again to solve cases where matrix hasen't been solved
        for (Integer i : notSolved)
        {
            //Set all ring bonds with single order to allow Matrix solving to work
            setAllRingBondsSingleOrder(ringGroups.get(i), ringSet);
...
//MFmod
//At this stage molecules should have all H in place
/*
            //Put in an implicit hydrogen atom for Planar3 C- atoms in 5-membered rings (it doesn't get put in by the Smiles parser)
            if (("C".equals(molecule.getAtom(j).getSymbol())) && (molecule.getAtom(j).getHybridization() == Hybridization.PLANAR3)) {
...
*/
            ...
            //Calculate free valencies available for extra ring bonding
// ORIGINAL
//            fvtc.add(molecule.getAtom(j).getValency() - (implicitH + (int) molecule.getBondOrderSum(molecule.getAtom(j))) + M.sumOfRow(i));

             int fv =     molecule.getAtom(j).getValency()
                        - implicitH 
                        - getBondOrderSum(molecule,molecule.getAtom(j))
                        + M.sumOfRow(i);
             fvtc.add(fv);
...
//MFmod
    /**
     * Get the sum of bond orders. Fixes the wrong bond order perception
     * for 
     */
    private int getBondOrderSum(IAtomContainer mol, IAtom atm)
    {
        int bos = (int) mol.getBondOrderSum(atm);

        //Look for metal coordination via aromatic N
        if (atm.getFlag(CDKConstants.ISAROMATIC) && atm.getSymbol().equals("N"))
        {
            List<IAtom> nbrs = mol.getConnectedAtomsList(atm);
            for (IAtom nbr : nbrs)
            {
                String s = nbr.getSymbol();
                if (PeriodicTable.isMetal(s))
                    bos--;
            }
        }
