 ...
 *
 * <p>This class was modified by Marco Foscato to allow setting of longer timeout to AllRingsFinder. 
 * This is achieved by adding a setRingFinderTimeout method and the arfTimeout variable. If the  
 * setRingFinderTimeout method is not called the SMARTSQueryTool behaves normally. The modified lines are 
 * between <code>MFmod</code> and <code>MFend</code> comments.</p>
 *
...
public class SMARTSQueryTool {
    ...
    //MFmod
    private long arfTimeout = -1;
    private int maxRingSize = -1;
    //MFend
    ...
    //MFmod
    /**
     * Set the timeout value in milliseconds of the AllRingsFinder
     *
     * @param timeout  The new timeout value
     */
    public void setAllRingsFinderTimeout(long timeout) {
        this.arfTimeout = timeout;
    }

    /**
     * Set the maximum ring size that will be considered by AllRingsFinder
     *
     * @param maxRingSize Maximum ring size to consider.
     */
    ...
    private void initializeMolecule() throws CDKException {
        ...
        try {
            //MFmod
            if (arfTimeout > 0) {
                arf.setTimeout(arfTimeout);
            }
            if (maxRingSize > 0) {
                allRings = arf.findAllRings(atomContainer,maxRingSize);
            } else {
