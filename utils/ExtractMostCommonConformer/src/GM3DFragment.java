...
{ 
    ...
    private static String checkfile = "check-GM3DFragment.sdf";
...
//------------------------------------------------------------------------------
    ...
    public IAtomContainer getContainerWithAPasAtoms()
    {
	return apOnMol;
    }
 ...
 * The comparison goes trhough a prioritized list of items according to the
 * following orther: number of atoms, number of bonds, number and type of 
 * attachment points, position of the attachment points on the molecular
 * structure.
 * TODO UPDATE documentation
 *
 * WARNING needs fixed aromaticity to work properly.
 *
 * @param other the <code>GM3DFragment</code> to be compared with this one
 * @return <code>true</code> if <code>this</code> is the same fragment of 
 * <code>other</code>
 */
...
//        for (int i = 0; i<likeAPs.size(); i++)
	if (likeAPs.size() == 1)
	    ...
	    int i = 0;
            if (repOnScreen >= 3)
                System.out.println(preComp+"Compating atom order of two fragments:");
	...
	{
	    if (repOnScreen >= 1)
                System.out.println(preComp+"INCHI representation FAILED. Fragment rejected! (see "+checkfile+")");
...
//------------------------------------------------------------------------------
    /**
     * Generated the INCHI code for this fragments using pseuso atoms
...
//TODO remove
//System.out.println("SIZE FLAG-1: "+flags.size());
        int apFlag = this.getFreeAtomsFlag();
//TODO remove
//System.out.println("SIZE FLAG-2: "+flags.size());
        int dummyFlag = this.getFreeAtomsFlag();
//TODO remove
//System.out.println("SIZE FLAG-3: "+flags.size());
...
//------------------------------------------------------------------------------
    ...
    public Map<String,String> getClassToElement(Set<String> classSet, IAtomContainer molA)
    {
        Map<String,String> c2e = new HashMap<String,String>();
//TODO D and T lead to problems in calculating atomic mass because 
// IsotopeFactory.getMajorIsotope doesn't seem to deal with dymbols D and T
        Set<String> oneCandEls = new HashSet<String>(Arrays.asList("He","Ne","Ar","Kr","Xe","Rn","Cs","Fr"));
        ...
        for (String apClass : classSet)
        {
            ...
            if (!foundOneFree)
                throw new Error("ERROR! Free Element not available for class-to-element.");
        }
        return c2e;
    }
...
//------------------------------------------------------------------------------
    ...
    public Map<String,String> getClassToElement(Set<String> classSet, IAtomContainer molA, IAtomContainer molB) 
    {
        Map<String,String> c2e = new HashMap<String,String>();
//TODO D and T lead to problems in calculating atomic mass because 
// IsotopeFactory.getMajorIsotope doesn't seem to deal with dymbols D and T
...
//------------------------------------------------------------------------------
	...
	{
//TODO now it's partially hard coded
    String moreAtmsSeparator = " ";
    String moreAPSeparator = ",";
    String atmSeparator = "#";
    String detailsSeparator = ":";
...
//
	    ...
	    String[] atomsPRop = allAtomsProp.split(moreAtmsSeparator);
	    for (int i = 0; i< atomsPRop.length; i++)
	    {
		String onThisAtm = atomsPRop[i];
		if (onThisAtm.contains(moreAPSeparator))
		{
		    String[] moreAPonThisAtm = onThisAtm.split(moreAPSeparator);
